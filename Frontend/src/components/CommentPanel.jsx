  ...
  const fetchComments = useCallback(
    async (pageToFetch) => {
      ...
      try {
        ...
        setComments((prev) =>
          pageToFetch === 1 ? data.comments : [...prev, ...data.comments]
        );
    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between p-3 border-b border-gray-200 sticky top-0 bg-white rounded-t-2xl">
          ...
          <button
            onClick={onClose}
            className="text-gray-500 hover:text-gray-800 absolute top-2 right-3"
          >
            <IoClose size={24} />
          </button>
        </div>
        ...
        <div ref={listRef} className="flex-grow overflow-y-auto p-4 space-y-4">
          {comments.map((comment, index) => {
            ...
            return (
              <div
                key={comment._id}
                ref={isLastElement ? lastCommentElementRef : null}
