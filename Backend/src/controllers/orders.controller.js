...
async function createOrder(req, res) {
  ...
  if (
    ...
    items.length === 0 ||
    !frontendTotalAmount ||
    typeof frontendTotalAmount !== "number" ||
    frontendTotalAmount <= 0 ||
    !deliveryAddress ||
    !paymentDetails ||
    ...
    return res
      ...
      .json({
        message:
          "Missing or invalid required order details (items array, totalAmount, deliveryAddress, paymentDetails.method, foodPartner ID).",
      });
  }
  ...
  if (
    deliveryAddress.type !== "Point" ||
    typeof deliveryAddress.address !== "object" ||
    (!deliveryAddress.address.formatted && !deliveryAddress.address.street)
  ) {
    return res
      .status(400)
      .json({ message: "Invalid delivery address structure." });
  }
  // Payment method validation
  if (!["Cash on Delivery", "UPI", "Card"].includes(paymentDetails.method)) {
    // Ensure 'Card' is in your enum
    return res
      .status(400)
      .json({ message: "Invalid payment method specified." });
  }
  ...
  try {
    ...
    if (!partnerExists) {
      return res
        .status(404)
        .json({ message: `Food partner with ID ${foodPartnerId} not found.` });
    }
    ...
    const foodIds = items.map((item) => {
      ...
      if (
        ...
        throw new Error(
          `Invalid quantity for item ${item.food}. Must be a positive integer.`
        );
      }
      return item.food;
    });
    ...
    const foodMap = new Map(
      foodItemsFromDB.map((food) => [food._id.toString(), food])
    );
    ...
    for (const item of items) {
      ...
      if (!foodDetails) {
        return res
          .status(404)
          .json({ message: `Food item with ID ${item.food} not found.` });
      }
      ...
      if (foodDetails.foodPartner.toString() !== foodPartnerId) {
        return res
          .status(400)
          .json({
            message: `Item ${foodDetails.name} (ID: ${item.food}) does not belong to the specified food partner.`,
          });
      }
    ...
    if (Math.abs(serverCalculatedTotal - frontendTotalAmount) > tolerance) {
      console.warn(
        `Order total mismatch. Frontend: ${frontendTotalAmount}, Server: ${serverCalculatedTotal}`
      );
      return res.status(400).json({
        message: `Order total amount mismatch. Expected ₹${serverCalculatedTotal.toFixed(
          2
        )} but received ₹${frontendTotalAmount.toFixed(
          2
    ...
    if (
      ...
      error.message.includes("Invalid food ID")
    ) {
      return res
        .status(400)
        .json({ message: "Order validation failed.", details: error.message });
    }
...
async function getOrderById(req, res) {
  ...
  try {
    const order = await Order.findById(orderId)
      ...
      .populate("foodPartner", "name location.address.formatted")
    ...
    const isPartnerOwner =
      req.foodPartner && order.foodPartner.equals(req.foodPartner._id);
